...
static SILENCE: [f32; MAX_QUANTUM] = [0.0; MAX_QUANTUM];
...
struct FilterState {
    ...
    music_l_port: *mut c_void,
    music_r_port: *mut c_void,
    voice_l_port: *mut c_void,
    voice_r_port: *mut c_void,
    output_l_port: *mut c_void,
    output_r_port: *mut c_void,
...
#[inline]
fn shift_and_append(window: &mut [f32], chunk: &[f32]) {
    let n = chunk.len();
    let len = window.len();
...
fn filter_state_name(s: pw_sys::pw_filter_state) -> &'static str {
...
unsafe extern "C" fn on_process(
    ...
    while offset < n {
        ...
        if state.samples_since_last_hop == hop_size {
            ...
            // Zero accumulators before this frame
            state.music_accum_l.fill(0.0);
            state.music_accum_r.fill(0.0);
            state.voice_accum_l.fill(0.0);
            state.voice_accum_r.fill(0.0);
...
pub fn run_pipewire(config: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut state = Box::new(FilterState::new(config));
